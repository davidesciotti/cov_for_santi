...
for nbl in (32, 64, 128):
    for weight_id in range(3):
        ...
        # ! ell binning
        ell_values, delta_values, ell_bin_edges = compute_ells(nbl, ell_min, ell_max, recipe='ISTF', output_ell_bin_edges=True)
        ...
        cl_3x2pt_1d = np.genfromtxt(f'{input_folder}/NoEll{nbl:03d}/dv-3x2pt-LiFE-C13-W{weight_id:02d}.dat')
        ...
        # not important, equivalent to the array above:
        ...
        if weight_id == 0 and nbl == 32:
            cov_bench = np.genfromtxt(
                f'../input/cl_LiFE/WeightedTest/NoEll{nbl:03d}/cm-3x2pt-LiFE-C13-W{weight_id:02d}.dat')
        ...
        np.savetxt(f'../output/WeightedTest/NoEll{nbl:03d}/cm-3x2pt-LiFE-C13-W{weight_id:02d}.dat', cov_3x2pt_2D, fmt='%.9e')
        ...
        np.savetxt(f'../output/WeightedTest/NoEll{nbl:03d}/ell_grid.txt', ell_grid, header=ell_grid_header)
        with open(f'../output/WeightedTest/NoEll{nbl:03d}/other_specs.txt', 'w') as file:
            file.write(json.dumps(other_quantities_tosave))

        print(f'case {nbl} bins, weights {weight_id} done')